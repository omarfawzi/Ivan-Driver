...
export default function FindRideScreen({ navigation }) {
  ...
  useEffect(async () => {
    setLoading(true)
    const pickUpStations = await stationController.getPickUpStations()
    setOriginStations(pickUpStations)
    await getLocation()
    setLoading(false)
  }, [])
    ...
    <View style={styles.container}>
      <MapView
        ...
        onLayout={() => {
          setTimeout(() => {
            mapRef.current.fitToSuppliedMarkers(
              originStations.map((station) => `station#${station.id}`),
              {
                edgePadding: { top: 50, right: 50, bottom: 50, left: 50 },
                animated: true,
              }
      ...
      {seats > 0 && origin && destination && selectedRoute ? (
        <View style={styles.overlayBottom}>
          <Fumi
            ...
            inputPadding={16}
            // editable={false}
            value={seats * fare + ' EGP'}
          />
          <Button
            style={{ width: 200, bottom: 30 }}
            mode="contained"
